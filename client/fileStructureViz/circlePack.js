...
const circlePack = objHierarchy => {
  ...
  const circle = g
    ...
    .attr('class', d => {
      ...
      if (
        !d.children &&
        d.data.name.match(/\.png|\.jpg|\.svg|\.mp4|\.webm|\.ogv/i)
      )
        cl = `${cl} media`;
    ...
    .attr('id', d => {
      return !d.children && d.parent.data.name === 'root'
        ? `${d.data.name.replace('.', 'DOT')}`
        : !d.children && d.parent.data.name !== 'root'
            ? `${d.parent.data.name.replace('.', 'DOT')}SLASH${d.data.name.replace('.', 'DOT')}`
            : null;
    })
  ...
  svg.style('background', '#fff').on('click', () => {
    zoom(root);
  });
